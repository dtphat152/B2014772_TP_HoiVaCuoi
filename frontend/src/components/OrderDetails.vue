    ...
    <div class="order-details">
        <div class="order-details-inner">
            ...
            <div class="d-flex flex-wrap h-50 flex-row" style="overflow-y: auto;">
                <div style="flex: 50%;" v-for="(f, index) in filterProducts" :key="f.product_id">
                    <div class="product-detail d-flex">
                        <div class="image">
                            <img :src="require(`../assets/images/${f.product_src}`)" alt="" />
                        </div>
                        <div class="content">
                            <p class="name">{{ f.product_name }} <span>X {{ item_qty[index] }}</span></p>
                            <p class="price">${{ f.product_price }}</p>
                        </div>
                    </div>
                </div>
            </div>
            <hr>
            <div class="price d-flex justify-content-center text-info font-weight-bold">
